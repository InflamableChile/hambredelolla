...
class FFDBManager extends LADBManager{
	...
	public function get_stream_settings(){
		$id = $_GET['stream-id'];
		die(json_encode(FFDB::getStream($this->streams_table_name, $id)));
	}
...
//			$stream->feeds = stripslashes($stream->feeds);//HACK
	...
	public function ff_save_settings_fn () {
		try{
			...
			FFDB::commit();
			echo json_encode( array('settings' => $settings, 'fb_extended_token' => $this->getOption('facebook_access_token')) ); // return serialized string back to client
