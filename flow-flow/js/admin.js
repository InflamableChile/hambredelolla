...
//				console.log('nextId', nextId);
				...
				html = templates.view.replace(/%id%/g, nextId).replace(/%plugin_url%/g, plugin_url);
				html = html.replace('%LIST%', '<tr><td  class="empty-cell" colspan="3">Add at least one feed</td></tr>').replace('%FEEDS%', '');
...
//			console.log('prepareStream')
			var stream = streamsData['id' + id];
			var $input;
			var feedsListStr = '', feedsStr = '', feeds, feed, info, prop, ikey, ival;
			var uid;
			var name, i, len;
			var $view, $t;
			var cpInit = false;
            ...
            var html = templates.view.replace( /%id%/g, id).replace(/%plugin_url%/g, plugin_url);
...
//							info = info + '<span class="highlight">' + ival + '</span>' ;
				...
				feedsListStr = '<tr><td  class="empty-cell" colspan="3">Add at least one feed</td></tr>';
				feedsStr = '';
			}
